class Animal:
    ...
    # point 类似于JAVA中的静态变量
    animal_count = 0
    _secret_food = 0  # point 前面增加了_后，变为类的protect变量
    __secretCount = 0  # point 前面增加了__后，变为类的private变量，对方法也是这样，两个__就表示私有
    ...
    # point 增加注解 @property 后，方法变成“只读属性”，调用时不加()
    @property
    def speak(self):
        print(f"动物叫了一声 我是{self.name} 动物总数{Animal.animal_count}")
        # del self.name #point 删除这个实例的name属性，但一般也不需要删除啊

    def get_age2(animal):
        return animal.age

    # point 添加注释 @classmethod 后，方法就变成了类方法，用来创建对象，操作类属性
    @classmethod
    def do_fun(cls):
        print(f"操作了类方法，{cls.animal_count}  {cls.speak}")
...
def test1():
    cat = Animal("cat", 1)
    cat.weight = 4  # point 动态添加属性
    cat.speak  # 写成speak()反而无法运行
...
def make_animal_speak(animal):
    print(animal.speak())
    print(animal.eat())  # point 瞎写方法也不会报错，只会在运行时检查
